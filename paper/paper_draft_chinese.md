...
- **有害提示池（v2）**：约5,000条。以 AdvBench[1] 为种子，经英语模板扩展至约3,500条现代有害表述；另含约1,500条**文言文 / CC-BOS 风格**规则合成样本（官职、典籍、隐喻映射等，脚本 `scripts/build_large_harmful_dataset.py` 可复现），用于覆盖「意图不变、表层文言包装」类攻击。
- Alpaca[7]：5,000条良性指令样本
- 自定义拒绝/顺从配对：用于引导方向计算（规模以 `data/raw/training_pairs/` 实际文件为准）

**评估数据**：

- 划分比例：训练集 / 验证集 / 测试集 = **70% / 15% / 15%**（在有害池5,000条与良性5,000条上分别划分）。例如有害侧测试集约 **750** 条、良性测试集约 **750** 条（具体数以运行 `preprocess_data.py` 后 `data/splits/test.json` 为准）。
...
*左图为测试集混淆矩阵（**旧版**520条有害池划分示例：TP=78, TN=741, FP=9, FN=0）；Dataset v2 有害池约5,000条、测试集约750条越狱×750条良性，**需重新训练后**更新矩阵与 TP/TN/FP/FN；右图为与基线方法的多维度性能对比。*
